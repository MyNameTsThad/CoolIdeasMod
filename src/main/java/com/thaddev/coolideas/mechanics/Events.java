        ...
        if (event.getEntity() instanceof ServerPlayer player && player.level.getServer() != null) {// just in case
            ...
            player.sendMessage(
                component(Utils.from("")).copy()
                    .append(Component.nullToEmpty("https://github.com/MyNameTsThad/CoolIdeasMod/blob/forge-119/README.md#ignore-if-you-did-not-come-from-an-in-game-chat-message").copy().withStyle(
                        Style.EMPTY
                    ...
                    .append(Component.nullToEmpty(" (versionid:" + CoolIdeasMod.buildVersionString(loader) + ")")),
                player.getUUID()
            );
        }
    }
